...
function Header(calendar) {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<div class="date_title_sub prev-2">&nbsp;</div><div class="date_title_sub prev-1">&nbsp;</div><div class="date_title">&nbsp;</div><div class="date_title_sub next-1">&nbsp;</div><div class="date_title_sub next-2">&nbsp;</div>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
	...
	function updateTitle(text) {
		if (el) {
			...
			el.find('.date_title_sub.prev-2').text(pt2).attr('data-date', py2 + '-' + pm2 + '-15');
			el.find('.date_title_sub.prev-1').text(pt1).attr('data-date', py1 + '-' + pm1 + '-15');
			el.find('.date_title').text(t);
			el.find('.date_title_sub.next-1').text(nt1).attr('data-date', ny1 + '-' + nm1 + '-15');
			el.find('.date_title_sub.next-2').text(nt2).attr('data-date', ny2 + '-' + nm2 + '-15');
		}
	}
